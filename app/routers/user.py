...
@router.get("/users/cursor", response_model=UserCursorResponse, status_code=status.HTTP_200_OK)
async def get_users_cursor(cursor: int, limit: int, db: AsyncSession = Depends(get_db)):
    try: 
        return await user.get_users_with_cursor(db, cursor, limit)
    except NoResultFound:
        raise HTTPException(status_code=404, detail="User not found")
    
@router.get("/users/list", response_model=UserCursorResponse, status_code=status.HTTP_200_OK)
async def get_users_list_from_search(data: str, limit: int, db: AsyncSession = Depends(get_db)):
    return await user.get_users_list_from_search(db, data, limit)